...
function GalleryScene({
    ...
    const textures = useTexture(normalizedImages.map((img: { src: string; alt?: string }) => img.src));
    ...
    const handleWheel = useCallback(
        (event: WheelEvent) => {
            setScrollVelocity((prev: number) => prev + event.deltaY * 0.01 * speed);
            setAutoPlay(false);
    ...
    const handleKeyDown = useCallback(
        (event: KeyboardEvent) => {
            if (event.key === 'ArrowUp' || event.key === 'ArrowLeft') {
                setScrollVelocity((prev: number) => prev - 2 * speed);
                setAutoPlay(false);
                lastInteraction.current = Date.now();
    ...
    useFrame((state: { clock: { getElapsedTime: () => number } }, delta: number) => {
        // Apply auto-play
        if (autoPlay) {
            setScrollVelocity((prev: number) => prev + 0.3 * delta);
        }
        ...
        planesData.current.forEach((plane: PlaneData, i: number) => {
    ...
    return (
        <>
            {planesData.current.map((plane: PlaneData, i: number) => {
    ...
    return (
        <div className="flex flex-col items-center justify-center h-full bg-gray-100 p-4">
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 gap-4 max-h-96 overflow-y-auto">
                {normalizedImages.map((img: { src: string; alt?: string }, i: number) => (
                    <img
                        key={i}
